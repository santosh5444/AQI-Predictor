...
# Pools for variety
winter_reasons = ['🌡️ Winter Temperature Inversion', '🏭 Sustained Industrial Plumes', '🏭 RINL Steel Flaring', '🏭 HPCL Refinery Emission Spike']
biomass_reasons = ['🔥 Post-Monsoon Biomass Burning', '💨 Stagnant Wind Speeds', '🌾 Agricultural Residue Fire', '🌫️ Regional Haze Transport']
summer_reasons = ['🏗️ Summer Heat & Dust Resuspension', '🚗 High Traffic Exhaust', '🚢 Port Operations Dust', '🏭 Industrial Power Surge']

for idx, row in true_spikes.head(15).iterrows():
    m = int(row['month'])
    causes = []
    # Seed based on date to keep it deterministic for the same dataset but varied across events
    np.random.seed(int(row['Timestamp'].timestamp()) % 1000)
    
    if m in [11, 12, 1, 2]: 
        causes = np.random.choice(winter_reasons, 2, replace=False).tolist()
    elif m in [10, 11]: 
        causes = np.random.choice(biomass_reasons, 2, replace=False).tolist()
    elif m in [3, 4, 5]: 
        causes = np.random.choice(summer_reasons, 2, replace=False).tolist()
    else: 
        causes = ['🏭 Localized Industrial Emission Spike', '🚗 Traffic Congestion Peak']
